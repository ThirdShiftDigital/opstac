...
document.getElementById('acctCallsignSave') && document.getElementById('acctCallsignSave').addEventListener('click', async () => {
  const val = ((document.getElementById('acctCallsign') || {}).value || '').trim() || null;
  if(!myPersonnel){
    alert('Your login is not linked to a roster card yet.');
    return;
  }
  const { data, error } = await savePersonnelRecord(myPersonnel.id, {
    callsign: val,
    unit_number: val
  }, false);
  if(error){
    alert('Could not save callsign: ' + error.message);
    return;
  }
  ...
  const hint = document.getElementById('acctRoleLabel');
  if(hint && val) hint.textContent = (hint.textContent || '').split(' · ')[0] + ' · ' + val;
});
...
async function onSignedIn(){
  ...
  const cs = document.getElementById('acctCallsign');
  if(cs) cs.value = (myPersonnel && (myPersonnel.callsign || myPersonnel.unit_number)) || '';
